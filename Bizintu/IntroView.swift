...
//  Bizintu
//
//  Created by Austin Wei on 12/13/17.
...
class IntroView: UIViewController, SegueHandler, GIDSignInUIDelegate, GIDSignInDelegate {
    ...
    @IBAction func emailSignUp(_ sender: UIButton) {
        ...
        delegate?.segueToNext(identifier: "EmailRegister")
    }
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        if segue.identifier == "embedMenu" {
            let dvc = segue.destination as! RegisterMenuController
            dvc.delegate = self
        }
    }
